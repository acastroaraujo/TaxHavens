...
```{r}
make_flows("China", 2012, "target", "inflow")
make_table("China", 2012, "target", "inflow")
make_flows("China", 2012, "source", "inflow")
make_table("China", 2012, "source", "inflow") %>%
  mutate(flow = round(flow, 2)) %>% 
  rename(Source = source, Target = target, 
         "Flow (millions of US dollars)" = flow) %>% 
